...
class TopsisHelper
{
    public static function getBobotByProfilRisiko($profilRisiko)
    {
        switch (strtolower($profilRisiko)) {
            case 'konservatif':
                return ['EPS' => 0.20, 'PER' => 0.50, 'ROE' => 0.30];
            case 'moderat':
                return ['EPS' => 0.35, 'PER' => 0.35, 'ROE' => 0.30];
            case 'agresif':
                return ['EPS' => 0.50, 'PER' => 0.15, 'ROE' => 0.35];
            default:
                return ['EPS' => 0.35, 'PER' => 0.35, 'ROE' => 0.30];
        }
    }
    public static function hitungTopsis($sahamData, $bobot)
    {
        ...
        foreach ($jarak as $index => $d) {
            $totalJarak = $d['positif'] + $d['negatif'];
            // Proteksi terhadap division by zero
            $skor = ($totalJarak > 0) ? ($d['negatif'] / $totalJarak) : 0;
    ...
    {
        $hasil = [];
        $kriteria = ['EPS', 'PER', 'ROE'];
        $pembagi = [];
        foreach ($kriteria as $k) {
            $sumKuadrat = 0;
            foreach ($matriks as $row) {
                $sumKuadrat += pow($row[$k], 2);
            }
        ...
        foreach ($matriks as $index => $row) {
            $hasil[$index] = [];
            foreach ($kriteria as $k) {
                $hasil[$index][$k] = $pembagi[$k] > 0 ? $row[$k] / $pembagi[$k] : 0;
            }
        }
        return $hasil;
    }
    ...
    public static function formatBobotToJson($bobot)
    {
        return json_encode($bobot);
    }
    public static function formatNormalisasiToJson($normalisasi)
    {
        return json_encode($normalisasi);
    }
