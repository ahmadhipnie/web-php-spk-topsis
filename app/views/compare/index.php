    ...
    <script>
        ...
        function renderComparisonTable(comparisonData, stocks) {
            ...
            stocks.forEach((stockCode, index) => {
                ...
                const safeSetContent = (id, content, isHTML = false) => {
                    const el = document.getElementById(id);
                    if (el) {
                        if (isHTML) {
                ...
                // Update header
                safeSetContent(`header${col}`, data.kode_saham);

                // Basic info
                safeSetContent(`code${col}`, data.kode_saham);
                safeSetContent(`name${col}`, data.nama_saham);
                safeSetContent(`sector${col}`, data.sektor || '-');
                safeSetContent(`price${col}`, formatCurrency(data.harga_terakhir), true);
                safeSetContent(`volume${col}`, formatVolume(data.volume));

                // Performance
                safeSetContent(`return${col}`, formatPercentage(data.return_pct), true);
                safeSetContent(`priceChange${col}`, formatPriceChange(data.price_change, data.price_change_pct), true);
                safeSetContent(`highest${col}`, formatCurrency(data.highest_price), true);
                safeSetContent(`lowest${col}`, formatCurrency(data.lowest_price), true);

                // Risk
                safeSetContent(`volatility${col}`, formatVolatility(data.volatility), true);
                safeSetContent(`riskAdjusted${col}`, data.risk_adjusted_return || '-');

                // Financial
                safeSetContent(`eps${col}`, data.eps || '-');
                safeSetContent(`per${col}`, data.per || '-');
                safeSetContent(`roe${col}`, data.roe ? data.roe + '%' : '-');
            });
        }
        ...
        function showWinner(comparisonData, stocks) {
            ...
            if (winner) {
                ...
                const winnerAlertEl = document.getElementById('winnerAlert');

                if (winnerTextEl && winnerAlertEl) {
