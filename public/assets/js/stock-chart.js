...
function renderPriceChart(chartData, info) {
    ...
    tooltip: {
      custom: function ({ seriesIndex, dataPointIndex, w }) {
        ...
        return `<div class="custom-tooltip">
                    <div class="tooltip-title">${date.toLocaleDateString(
                      "id-ID"
                    )}</div>
  ...
  // Render new chart
  priceChartInstance = new ApexCharts(
...
function renderVolumeChart(chartData) {
  ...
  // Render new chart
  volumeChartInstance = new ApexCharts(
...
function updateChartInfo(chartData, info) {
  // Total data
  document.getElementById("infoTotalData").textContent =
    info.total_data + " hari";
  ...
  document.getElementById("infoHighest").textContent =
    "Rp " + highest.toLocaleString("id-ID");
  document.getElementById("infoLowest").textContent =
    "Rp " + lowest.toLocaleString("id-ID");

  const changeEl = document.getElementById("infoChange");
  const changeText = (change >= 0 ? "+" : "") + change.toFixed(2) + "%";
  changeEl.textContent = changeText;
  changeEl.className =
    "info-value " + (change >= 0 ? "text-success" : "text-danger");
}

/**
 * Download stock data as CSV
 */
function downloadStockCSV(kode_saham, periode) {
